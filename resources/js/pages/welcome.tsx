// import banner from '@/assets/banner.jpg';
// import Dummy from '@/assets/example.png';
// import banner2 from '@/assets/example2.png';
// import { Container } from '@/components/Container';
// import { type SharedData } from '@/types';
// import { Head, usePage } from '@inertiajs/react';
// import { Swiper, SwiperSlide } from 'swiper/react';

// // Import Swiper styles
// import 'swiper/css';
// import 'swiper/css/effect-fade';
// import 'swiper/css/navigation';
// import 'swiper/css/pagination';

// // import required modules
// import { Footer } from '@/components/blog/Footer';
// import { Navbar } from '@/components/blog/Navbar';
// import { Button } from '@/components/ui/button';
// import { Card } from '@/components/ui/card';
// import { Clock, Send } from 'lucide-react';
// import { useState } from 'react';
// import { Autoplay, EffectFade, Navigation, Pagination } from 'swiper/modules';

// const bannerData = [{ image: banner2 }, { image: banner }];

// export default function Welcome() {
//     const { auth } = usePage<SharedData>().props;
//     const [isOpen, setIsOpen] = useState(false);
//     const [photoIndex, setPhotoIndex] = useState(0);

//     console.log(auth);
//     return (
//         <>
//             <Head title="Beranda">
//                 <link rel="preconnect" href="https://fonts.bunny.net" />
//                 <link
//                     href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600"
//                     rel="stylesheet"
//                 />
//             </Head>
//             <header className="m-0 mb-4 bg-green-700 text-white shadow-2xl">
//                 <div></div>
//                 <Navbar />
//             </header>
//             <main>
//                 <Container>
//                     {/* Banner */}
//                     <section className="px-2">
//                         <Swiper
//                             // navigation={true}
//                             pagination={{
//                                 dynamicBullets: true,
//                                 clickable: true,
//                             }}
//                             spaceBetween={50}
//                             loop={true}
//                             effect={'fade'}
//                             autoplay={{
//                                 delay: 3000,
//                                 disableOnInteraction: false,
//                             }}
//                             modules={[Navigation, Pagination, EffectFade, Autoplay]}
//                             className="aspect-video w-full rounded-xl shadow-xl lg:aspect-16/6"
//                         >
//                             {bannerData.map((value, index) => (
//                                 <SwiperSlide key={index}>
//                                     <div className="h-full bg-amber-200 text-center align-middle">
//                                         <img
//                                             src={value.image}
//                                             alt=""
//                                             className="h-full w-full object-fill brightness-90"
//                                         />
//                                     </div>
//                                 </SwiperSlide>
//                             ))}
//                         </Swiper>
//                     </section>
//                     {/* Banner End */}
//                 </Container>
//                 {/* Top Rencent */}
//                 <section className="justify-center py-15">
//                     <Container className="px-6 xl:px-0">
//                         <div className="flex items-center justify-between">
//                             <h1 className="relative h-fit w-auto text-2xl font-extrabold text-green-700 after:absolute after:-bottom-3 after:left-0 after:h-1 after:w-[60px] after:rounded-2xl after:bg-green-500 after:content-['']">
//                                 Artikel Terbaru
//                             </h1>
//                             <Button className="bg-green-600">
//                                 Lihat Lainnya <Send />
//                             </Button>
//                         </div>
//                         <div className="py-10">
//                             <div className="grid grid-rows-3 gap-4 lg:grid-flow-col">
//                                 <div className="row-span-3 w-full sm:col-span-1 xl:max-w-[500px]">
//                                     <Card className="group relative aspect-square cursor-pointer overflow-hidden p-0">
//                                         <img
//                                             src={Dummy}
//                                             alt="image"
//                                             className="relative aspect-square object-fill brightness-100 transition-transform duration-300 group-hover:scale-110"
//                                         />
//                                         <div className="absolute bottom-0 m-4 rounded-lg bg-transparent p-5 text-left text-black shadow-2xl backdrop-blur-md">
//                                             <div className="flex items-center gap-2 pb-2 text-xs font-extralight text-green-900 lg:text-sm">
//                                                 <Clock className="size-4" /> 3 mins ago
//                                             </div>
//                                             <h3 className="text-md font-bold lg:text-xl">
//                                                 Lorem ipsum dolor sit amet consectetur adipisicing
//                                                 elit. Sunt, distinctio.
//                                             </h3>
//                                         </div>
//                                         <div className="absolute flex flex-wrap gap-2 px-4 py-5">
//                                             <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                 Aqidah
//                                             </span>
//                                             <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                 Pemuda
//                                             </span>
//                                             <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                 Fiqih
//                                             </span>
//                                         </div>
//                                     </Card>
//                                 </div>
//                                 <div className="col-span-1 max-w-3xl lg:col-span-0">
//                                     <Card className="group flex cursor-pointer flex-row gap-1 overflow-hidden p-0 lg:gap-3">
//                                         <img
//                                             src={Dummy}
//                                             alt=""
//                                             className="aspect-square w-30 rounded-xl transition-transform duration-300 group-hover:scale-110 lg:w-38"
//                                         />
//                                         <div className="p-2">
//                                             <div className="flex items-center gap-1 px-1 py-2">
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Fiqih
//                                                 </span>
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Pemuda
//                                                 </span>
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Aqidah
//                                                 </span>
//                                             </div>
//                                             <h3 className="text-md font-bold lg:text-lg">
//                                                 Lorem ipsum dolor sit amet, consectetur adipisicing
//                                                 elit. Eos, placeat.
//                                             </h3>
//                                             <div className="hidden items-center gap-2 py-1 text-xs font-extralight text-gray-600 lg:flex">
//                                                 <Clock className="size-4" /> 3 mins ago
//                                             </div>

//                                             <p className="hidden py-2 font-sans font-extralight lg:block">
//                                                 Lorem ipsum dolor, sit amet consectetur adipisicing
//                                             </p>
//                                         </div>
//                                     </Card>
//                                 </div>
//                                 <div className="col-span-1 max-w-3xl lg:col-span-0">
//                                     <Card className="group flex cursor-pointer flex-row gap-1 overflow-hidden p-0 lg:gap-3">
//                                         <img
//                                             src={Dummy}
//                                             alt=""
//                                             className="aspect-square w-30 rounded-xl transition-transform duration-300 group-hover:scale-110 lg:w-38"
//                                         />
//                                         <div className="p-2">
//                                             <div className="flex items-center gap-1 px-1 py-2">
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Fiqih
//                                                 </span>
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Pemuda
//                                                 </span>
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Aqidah
//                                                 </span>
//                                             </div>
//                                             <h3 className="text-md font-bold lg:text-lg">
//                                                 Lorem ipsum dolor sit amet, consectetur adipisicing
//                                                 elit. Eos, placeat.
//                                             </h3>
//                                             <div className="hidden items-center gap-2 py-1 text-xs font-extralight text-gray-600 lg:flex">
//                                                 <Clock className="size-4" /> 3 mins ago
//                                             </div>

//                                             <p className="hidden py-2 font-sans font-extralight lg:block">
//                                                 Lorem ipsum dolor, sit amet consectetur adipisicing
//                                             </p>
//                                         </div>
//                                     </Card>
//                                 </div>
//                                 <div className="col-span-1 max-w-3xl lg:col-span-0">
//                                     <Card className="group flex cursor-pointer flex-row gap-1 overflow-hidden p-0 lg:gap-3">
//                                         <img
//                                             src={Dummy}
//                                             alt=""
//                                             className="aspect-square w-30 rounded-xl transition-transform duration-300 group-hover:scale-110 lg:w-38"
//                                         />
//                                         <div className="p-2">
//                                             <div className="flex items-center gap-1 px-1 py-2">
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Fiqih
//                                                 </span>
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Pemuda
//                                                 </span>
//                                                 <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                                     Aqidah
//                                                 </span>
//                                             </div>
//                                             <h3 className="text-md font-bold lg:text-lg">
//                                                 Lorem ipsum dolor sit amet, consectetur adipisicing
//                                                 elit. Eos, placeat.
//                                             </h3>
//                                             <div className="hidden items-center gap-2 py-1 text-xs font-extralight text-gray-600 lg:flex">
//                                                 <Clock className="size-4" /> 3 mins ago
//                                             </div>

//                                             <p className="hidden py-2 font-sans font-extralight lg:block">
//                                                 Lorem ipsum dolor, sit amet consectetur adipisicing
//                                             </p>
//                                         </div>
//                                     </Card>
//                                 </div>
//                             </div>
//                         </div>
//                     </Container>
//                 </section>
//                 {/* Top Rencent End*/}
//                 {/* Poster */}
//                 <section className="bg-green-700 py-15">
//                     <Container className="text-center">
//                         <h3 className="relative w-auto text-2xl font-extrabold text-green-50 after:absolute after:-bottom-3 after:left-1/2 after:h-1 after:w-[70px] after:-translate-x-1/2 after:rounded-2xl after:bg-green-500 after:content-['']">
//                             Galeri Poster Dakwah
//                         </h3>
//                         <div className="px-5 py-15">
//                             <Swiper
//                                 pagination={{
//                                     dynamicBullets: true,
//                                     clickable: true,
//                                 }}
//                                 breakpoints={{
//                                     0: { slidesPerView: 1 },
//                                     640: { slidesPerView: 2 },
//                                     1024: { slidesPerView: 4 },
//                                 }}
//                                 spaceBetween={30}
//                                 loop={true}
//                                 autoplay={{
//                                     delay: 2500,
//                                     disableOnInteraction: false,
//                                 }}
//                                 modules={[Navigation, Pagination, EffectFade, Autoplay]}
//                                 className="h-auto w-full"
//                             >
//                                 {Array.from({ length: 10 }).map((_, index) => (
//                                     <SwiperSlide key={index}>
//                                         <div className="grid aspect-square cursor-pointer place-items-center justify-center rounded-2xl bg-amber-200">
//                                             {index + 1}
//                                         </div>
//                                     </SwiperSlide>
//                                 ))}
//                             </Swiper>
//                         </div>
//                     </Container>
//                 </section>
//                 {/* Poster End */}

//                 {/* Question */}
//                 <section className="pt-15">
//                     <Container className="px-3">
//                         <h3 className="relative h-fit w-fit text-2xl font-extrabold text-green-700 after:absolute after:-bottom-3 after:left-0 after:h-1 after:w-[60px] after:rounded-2xl after:bg-green-500 after:content-['']">
//                             Problematika Ummat
//                         </h3>
//                         <div className="grid gap-4 py-10 lg:grid-cols-2">
//                             <Card className="group cursor-pointer p-4 transition-all duration-200 hover:scale-102 hover:border-green-600">
//                                 <div>
//                                     <div className="flex items-center gap-1 px-1 py-2">
//                                         <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                             Tanya Jawab
//                                         </span>
//                                     </div>
//                                     <h1 className="text-lg font-semibold">
//                                         Lorem ipsum dolor sit amet consectetur, adipisicing elit.
//                                         Doloremque, culpa?
//                                     </h1>
//                                     <span className="flex items-center gap-2 py-1 text-sm text-gray-400">
//                                         <Clock className="size-4" /> 3 times ago
//                                     </span>
//                                     <p className="text-justify">
//                                         Lorem ipsum dolor sit amet consectetur adipisicing elit.
//                                         Sint consequatur atque molestiae commodi consectetur
//                                         adipisci saepe eum ut at dolorum.
//                                     </p>
//                                 </div>
//                             </Card>
//                             <Card className="group cursor-pointer p-4 transition-all duration-200 hover:scale-102 hover:border-green-600">
//                                 <div>
//                                     <div className="flex items-center gap-1 px-1 py-2">
//                                         <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                             Tanya Jawab
//                                         </span>
//                                     </div>
//                                     <h1 className="text-lg font-semibold">
//                                         Lorem ipsum dolor sit amet consectetur, adipisicing elit.
//                                         Doloremque, culpa?
//                                     </h1>
//                                     <span className="flex items-center gap-2 py-1 text-sm text-gray-400">
//                                         <Clock className="size-4" /> 3 times ago
//                                     </span>
//                                     <p className="text-justify">
//                                         Lorem ipsum dolor sit amet consectetur adipisicing elit.
//                                         Sint consequatur atque molestiae commodi consectetur
//                                         adipisci saepe eum ut at dolorum.
//                                     </p>
//                                 </div>
//                             </Card>
//                             <Card className="group cursor-pointer p-4 transition-all duration-200 hover:scale-102 hover:border-green-600">
//                                 <div>
//                                     <div className="flex items-center gap-1 px-1 py-2">
//                                         <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                             Tanya Jawab
//                                         </span>
//                                     </div>
//                                     <h1 className="text-lg font-semibold">
//                                         Lorem ipsum dolor sit amet consectetur, adipisicing elit.
//                                         Doloremque, culpa?
//                                     </h1>
//                                     <span className="flex items-center gap-2 py-1 text-sm text-gray-400">
//                                         <Clock className="size-4" /> 3 times ago
//                                     </span>
//                                     <p className="text-justify">
//                                         Lorem ipsum dolor sit amet consectetur adipisicing elit.
//                                         Sint consequatur atque molestiae commodi consectetur
//                                         adipisci saepe eum ut at dolorum.
//                                     </p>
//                                 </div>
//                             </Card>
//                             <Card className="group cursor-pointer p-4 transition-all duration-200 hover:scale-102 hover:border-green-600">
//                                 <div>
//                                     <div className="flex items-center gap-1 px-1 py-2">
//                                         <span className="rounded-md bg-green-600 px-2 py-1 text-xs font-medium text-white shadow-xl">
//                                             Tanya Jawab
//                                         </span>
//                                     </div>
//                                     <h1 className="text-lg font-semibold">
//                                         Lorem ipsum dolor sit amet consectetur, adipisicing elit.
//                                         Doloremque, culpa?
//                                     </h1>
//                                     <span className="flex items-center gap-2 py-1 text-sm text-gray-400">
//                                         <Clock className="size-4" /> 3 times ago
//                                     </span>
//                                     <p className="text-justify">
//                                         Lorem ipsum dolor sit amet consectetur adipisicing elit.
//                                         Sint consequatur atque molestiae commodi consectetur
//                                         adipisci saepe eum ut at dolorum.
//                                     </p>
//                                 </div>
//                             </Card>
//                         </div>
//                         <div className="w-full pb-15 text-center">
//                             <Button className="bg-green-600">
//                                 Lihat Lainnya <Send />
//                             </Button>
//                         </div>
//                         <div className="flex w-full items-center justify-between rounded-t-xl bg-green-600 px-6 py-7 lg:px-10 lg:py-15">
//                             <h3 className="font-sans text-xl font-bold text-white lg:text-4xl">
//                                 Ada hal yang ingin ditanyakan?
//                             </h3>
//                             <Button className="cursor-pointer rounded-4xl border-2 border-white bg-transparent py-6 font-semibold transition-all duration-400 hover:-translate-y-2 hover:bg-white hover:text-green-600 lg:px-9 lg:py-7 lg:text-xl">
//                                 Tanya Asatidz
//                             </Button>
//                         </div>
//                     </Container>
//                 </section>
//                 {/* Question End*/}
//             </main>
//             <Footer />
//         </>
//     );
// }
